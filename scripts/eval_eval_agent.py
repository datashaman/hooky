...
def deterministic_case_eval(
    ...
    if changed:
        findings.append(f"Eval Agent modified files outside its report area: {changed}")
...
def workspace_hashes(root: Path) -> dict[str, str]:
    ...
    for path in sorted(root.rglob("*")):
        if path.is_file() and not is_eval_artifact(path, root):
...
def is_eval_artifact(path: Path, root: Path) -> bool:
    relative = path.relative_to(root)
    return len(relative.parts) >= 3 and relative.parts[:3] == (".workflow", "artifacts", "eval-agent")
