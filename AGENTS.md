...
1. Planner turns a proposal into initial proposal/contract material and never edits product code.
...
2. Reason by writing or revising `.hooky/runs/<key>/contract.md`.
...
- Each role consumes durable files from `.hooky/runs/<key>/` and the current workspace state.
...
- Evaluator may read files, run tests, launch browsers, inspect screenshots, and emit evaluator reports.
- Evaluator must not edit implementation or test files.
- System code owns `.hooky/current.json`, run state, log append operations, attempt bookkeeping, evaluator report files, and command orchestration.
